...
class GamePlay extends JPanel implements KeyListener, ActionListener {
	...
	public void paint(Graphics g) {
		...
		g.setColor(new Color(128, 70, 27)); // bar color
		g.fillRect(playerX, 550, 100, 12);

		g.setColor(new Color(128, 70, 27)); // ball color
		g.fillOval(ballposX, ballposY, 20, 20);

		g.setColor(Color.black);
		g.setFont(new Font("Helvetica Bold", Font.BOLD, 25));
		...
		if (totalBricks <= 0) { // if all bricks are destroyed then you win
			...
			g.setFont(new Font("Baskerville", Font.BOLD, 17));
		...
		if (ballposY > 570) { // if ball goes below the paddle then you lose
			...
			g.drawString("Score: " + score, 270, 340);
			g.setFont(new Font("Stone Sans Sem ITC TT", Font.BOLD, 17));
			g.drawString("Press Enter to Restart", 250, 370);
